...
where CSVFILE is a comma-separated file with columns:
    ...
    - `Output Modality  (Speech or Write or Type)
    """)
    sys.exit(1)
...
def display_instructions(instructions_folder, INSTRUCTIONS):
    instruction_image_path = Path(instructions_folder) / f"{INSTRUCTIONS}.png"
...
for i, trial in enumerate(b.trials):
    ...
    # Add INTERBLOCK_DURATION every 5 stimuli except the last block
    if (i + 1) % 5 == 0 and (i + 1) < len(b.trials) - 1:
...
exp.keyboard.wait_char(CONTROLLER_KEY)
display_instructions(Path(instruction_image_folder), 'Type')
exp.keyboard.wait_char(CONTROLLER_KEY)
